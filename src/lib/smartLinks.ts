...
const DOMAIN_RULES: Record<string, (url: string, lang: SearchLanguage) => string> = {
  "google.com": (url, lang) => {
...
export function getSmartUrl(template: string, query: string, lang: SearchLanguage): string {
  let finalUrl = template;
  const q = encodeURIComponent(query);

  if (finalUrl.includes("{raw}")) finalUrl = finalUrl.replace("{raw}", query);
  else if (finalUrl.includes("{plus}")) finalUrl = finalUrl.replace("{plus}", query.trim().replace(/\s+/g, "+"));
  else if (finalUrl.includes("{query}")) finalUrl = finalUrl.replace("{query}", q);
