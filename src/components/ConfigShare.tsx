...
export default function ConfigShare({
  ...
  providers: CustomProvider[];
  onApply: (next: { providers: CustomProvider[]; externalSites: ExternalSite[] }) => void;
  ...
  function downloadJson() {
    const blob = new Blob([JSON.stringify(toObject, null, 2)], { type: "application/json;charset=utf-8" });
  ...
  return (
    <section className="panel">
      <h3 className="font-semibold mb-2">{t("cfg.title")}</h3>
      <div className="flex flex-wrap items-center gap-2">
        <button className="btn-ghost btn-sm" onClick={downloadJson}>{t("cfg.export")}</button>
        <button className="btn-ghost btn-sm" onClick={copyLink}>{t("cfg.copyLink")}</button>
        <button className="btn-ghost btn-sm" onClick={openFileDialog}>{t("cfg.importFile")}</button>
      </div>
      ...
      <div className="mt-3 grid md:grid-cols-[1fr_auto] gap-2">
        <input
          ...
          className="field"
        />
        <button className="btn-primary btn-sm" onClick={importFromText}>{t("cfg.apply")}</button>
      </div>
      <p className="text-xs text-muted mt-2">{t("cfg.desc")}</p>
    </section>
  );
}
