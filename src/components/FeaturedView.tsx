...
export default function FeaturedView() {
  ...
  const loadTopic = async (topic: typeof TOPICS[0], forceRefresh = false) => {
    if (!forceRefresh && topic.id === activeTopic.id && books.length > 0) return;
    setLoading(true);
    ...
    try {
      ...
      const withCover = results.filter((b: any) => b.cover);
  ...
  useEffect(() => { loadTopic(TOPICS[0], true); }, []);
  ...
  return (
    <div className="flex flex-col gap-8 py-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2 -mx-4 px-4 md:mx-0 md:px-0 md:pb-0 scrollbar-hide">
          {TOPICS.map((t) => {
            ...
            return (
              <button
                key={t.id}
                onClick={() => loadTopic(t, true)}
                className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all whitespace-nowrap border ${isActive ? "bg-brand text-white border-brand shadow-md shadow-brand/25 scale-105" : "bg-card text-muted-foreground border-border hover:bg-muted hover:text-foreground"}`}
              >
                <Icon size={14} />
                {t.label}
              </button>
      ...
      <div className="min-h-[300px]">
        {/* IMPORTANTE: lg:grid-cols-2 em vez de 3 para dar espaço aos cartões horizontais */}
        {loading ? (
